...
(define normal-material
  (Material
   (lambda ([ray : Ray][pt : Float3][norm : Float3])
    (local
      {(define norm_color : RGB (RGB
                                        (* 0.5 (+ 1.0 (Float3-x norm)))
                                        (* 0.5 (+ 1.0 (Float3-y norm)))
                                        (* 0.5 (+ 1.0 (Float3-z norm)))))}
  (make-reflect-info norm_color (Ray fl3-zero fl3-zero))))))
  ;;no reflection, so use
...
(define (lambertian-material rgb)
  (Material (lambda ([ray : Ray] [pt : Float3] [norm : Float3])   

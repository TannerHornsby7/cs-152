...
(define (hit-test obj min-t ray)
  ((Object-hit-test obj) min-t ray))
...
;;takes a list of objects and returns aggregate object
(define (list->object object_list)
  (Object
   (lambda ([ray : Ray] [min-t : Float])
     (local
       {(: Object->Hit-List : Object Hit-List -> Hit-List)
        (define (Object->Hit-List obj1 hl)
        (merge-hit-lists ((Object-hit-test obj1) ray min-t) hl))}        
     (foldl Object->Hit-List miss object_list)
     ))))


;;  (match object_list
;;    [(cons x '()) x]
;;    [_ (foldl merge-hit-lists object_list)]) )
